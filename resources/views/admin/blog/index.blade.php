...
<div class="container mx-auto mt-5">
    <div class="overflow-x-auto w-full border shadow-lg rounded-lg p-4 bg-white dark:bg-gray-800">
        <h3 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Section Blog</h3>
        <button onclick="window.location.href='{{ route('admin.blogadd') }}'" class="flex items-center py-2.5 px-6 rounded-lg text-sm font-medium text-white bg-blue-600 mt-5 mb-5 hover:bg-blue-700 transition duration-200">
        ...
        <div class="overflow-hidden">
            <table class="min-w-full rounded-xl table-auto">
                ...
                <tbody class="divide-y divide-gray-300">
                    @foreach($blog as $index => $berita)
                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                        <td class="p-4 text-center">{{ $index + 1 }}</td>
                        <td class="p-4 text-center">{{ $berita->judul }}</td>
                        <td class="p-4 text-center">
                            <img src="{{ asset('storage/' . $berita->foto) }}" alt="Foto" class="w-24 h-24 object-cover mx-auto">
                        </td>
                        <td class="p-4 text-center">{{ Str::limit($berita->body, 100) }}</td>
                        <td class="p-4 text-center flex pt-10 gap-4">
                            <a href="{{ route('admin.blogedit', $berita->id) }}" class="text-indigo-500 hover:text-indigo-900">
                            ...
                            <form action="{{ route('admin.blogdestroy', $berita->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
